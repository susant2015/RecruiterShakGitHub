...
+(ForgotPasswordService*)sharedInstance{
    ...
    static ForgotPasswordService *signinService=nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        signinService=[[ForgotPasswordService alloc] initWithService:FORGOT_PASSWORD];
    });
    return signinService;
    
}
-(void)ForgotPasswordServiceForEmailName:(NSString *)strEmailName    withCompletionHandler:(CompletionHandler)handler{
    NSString *post = [[NSString alloc] initWithFormat:@"action=%@&email=%@",@"forgot_password",strEmailName];
