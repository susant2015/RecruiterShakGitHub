...
@implementation EmployerJobListService

+(EmployerJobListService*)sharedInstance{
    
    static EmployerJobListService *employerJobListService=nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        employerJobListService=[[EmployerJobListService alloc] initWithService:Employer_UserId];
    });
    return employerJobListService;
    
}
-(void)employerJobListUserId:(NSString *)strUserId  withCompletionHandler:(CompletionHandler)handler{
    
    NSString *post = [[NSString alloc] initWithFormat:@"action=%@&u_id=%@",@"job_listing",strUserId];
